declare module "*/lib/enonic/static" {
  namespace staticLib {
    interface StaticLibrary {
       ...
       */
      buildGetter(root: string, params?: BuildGetterParams): (req: XP.Request) => XP.Response;

      /**
       * Sets up and returns a reusable resource-getter function.
       */
      buildGetter(params: BuildGetterParams & { root: string }): (req: XP.Request) => XP.Response;

      /**
       * A specific-recource getter method, returns a response object for the particular asset that’s named in the
       * argument string.
       */
      get(path: string, params?: GetParams): XP.Response;

      /**
       * A specific-recource getter method, returns a response object for the particular asset that’s named in the
       * argument string.
       */
      get(params: GetParams & { path: string }): XP.Response;
    }
    ...
    export type CacheControlResolver = (
      filePathAndName?: string,
      resource?: import("/lib/xp/io").Resource,
      mimeType?: string
    ) => string | null;
  ...
  const staticLib: staticLib.StaticLibrary;
  export = staticLib;
}
