...
export interface StaticLibrary {
   ...
   */
  buildGetter(root: string, params?: BuildGetterParams): (req: Request) => Response;

  /**
   * Sets up and returns a reusable resource-getter function.
   */
  buildGetter(params: BuildGetterParams & { root: string }): (req: Request) => Response;

  /**
   * A specific-recource getter method, returns a response object for the particular asset that’s named in the
   * argument string.
   */
  get(path: string, params?: GetParams): Response;

  /**
   * A specific-recource getter method, returns a response object for the particular asset that’s named in the
   * argument string.
   */
  get(params: GetParams & { path: string }): Response;
}
